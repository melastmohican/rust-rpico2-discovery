...
//!
//! ```bash
//! cargo run --example zermattb_snow
//! ```
...
fn main() -> ! {
    ...
    // Control pins
    let cs = pins.gpio17.into_push_pull_output(); // TCS - Chip Select
    let dc = pins.gpio20.into_push_pull_output(); // DC - Data/Command
    let mut rst = pins.gpio21.into_push_pull_output(); // RST - Reset

    // Create SPI bus with a safer clock speed for jumper wires
    ...
    info!("SPI configured at 24 MHz for stability");
...
fn render_flake(display: &mut impl DrawTarget<Color = Rgb565>, row: usize, col: usize) {
    ...
    // Draw a small 2x2 white square as a single solid fill (much faster and more stable)
    let rect = Rectangle::new(
        Point::new(x, y),
        Size::new(FLAKE_SIZE as u32, FLAKE_SIZE as u32),
    );
    display.fill_solid(&rect, SNOW_COLOR).ok();
}
...
fn render_void(
    ...
    if let Ok(bmp) = Bmp::<Rgb565>::from_slice(bmp_data) {
        ...
        // Write all 4 pixels in one contiguous chunk
        let rect = Rectangle::new(
            Point::new(x, y),
            Size::new(FLAKE_SIZE as u32, FLAKE_SIZE as u32),
        );
        display.fill_contiguous(&rect, colors).ok();
    }
}
